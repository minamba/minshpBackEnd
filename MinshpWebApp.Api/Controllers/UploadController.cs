    ...
    public class UploadController : Controller
    {
        ...
        public async Task<IActionResult> UploadDocument([FromForm] UploadRequest request)
        {
            ...
            if (folder == images)
            {
                ...
                if (requestType == add)
                    _imageViewModelBuilder.AddImagesAsync(new ImageRequest { Description = request.Description, Url = fileUrl, IdProduct = request.IdProduct, Title = request.Title, Position = request.Position });
                else
                    _imageViewModelBuilder.UpdateImagesAsync(new ImageRequest { Id = request.Id, Description = request.Description, Url = fileUrl, IdProduct = request.IdProduct, Title = request.Title, Position = request.Position });

            }
            else
            {
                if (requestType == add)
                    _videoViewModelBuilder.AddVideoAsync(new VideoRequest { Description = request.Description, Url = fileUrl, IdProduct = request.IdProduct, Title = request.Title, Position = request.Position });
                else
                    _videoViewModelBuilder.UpdateVideoAsync(new VideoRequest { Id = request.Id, Description = request.Description, Url = fileUrl, IdProduct = request.IdProduct, Title = request.Title, Position = request.Position });
            }
